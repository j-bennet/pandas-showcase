...
zp = pd.read_fwf(
    ...
    names=['zip_code', 'state', 'city', 'type', 'county_fips',
           'lat', 'lon', 'area_code', 'fin_code', 'last_line',
           'facility', 'msa', 'pmsa', 'filler'],
...
ms = pd.read_fwf(
    'data/MSA.DAT',
    widths=[4, 4, 60, 2, 8],
    names=['code', 'type', 'name', 'cmsa', 'population'],
    usecols=[0, 1, 2, 3],
    converters={'code': str, 'type': str, 'name': str, 'cmsa': str},
    header=None,
    skiprows=2
)
...
# Select only zip code and msa code
zip_codes = zp[["zip_code", "msa"]]

# Rename "code" to "msa_code"
zip_codes = zip_codes.rename(columns={'msa': 'msa_code'})

# Select only MSA code and MSA name
msa_codes = ms[['code', 'name']]
...
engine = create_engine('postgresql://username:password@localhost:5432/playground')

zip_codes.to_sql('zip_code', engine)
msa_codes.to_sql('msa', engine)